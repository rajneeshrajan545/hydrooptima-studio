...
with col1:
    ...
    speed = st.slider("Service Speed (kn)", 10.0, 22.0, 14.0, 0.5)
...
with col2:
    ...
    fig.add_trace(go.Scatter3d(x=px, y=py, z=pz, mode='markers', marker=dict(color='gold', size=4)))
    rx, ry, rz = generate_universal_rudder("Asymmetric Twisted Leading-Edge", span, chord, 0.18)
    fig.add_trace(go.Scatter3d(x=rx.flatten(), y=ry.flatten(), z=rz.flatten(), mode='markers', marker=dict(color='teal', size=4)))
