...
def generate_universal_propeller(diameter, hub_ratio, blades, pitch_law, wake):
    ...
    for b in range(blades):
        ...
        for r in np.linspace(r_hub, radius, 8):
            norm_r = r / radius
            pitch = (1.0 - 0.15 * (norm_r - 0.7)**2) * (1.0 - wake) if pitch_law == "Parabolic" else (1.1 - 0.2 * norm_r) * (1.0 - wake)
...
if check_password():
    ...
    with col2:
        ...
        st.header("🔮 Geometry Preview & Cavitation Matrix")
        fig = go.Figure()
        px, py, pz = generate_universal_propeller(diam, 0.22, blades, "Parabolic", 0.2)
        ...
        fig.update_layout(template="plotly_dark", height=400, scene=dict(aspectmode='data'))
        st.plotly_chart(fig, use_container_width=True)
        
        st.subheader("📉 Dynamic IMO CII Regulatory Timeline")
        years = [2026, 2027, 2028, 2029, 2030]
        fig_cii = go.Figure()
        fig_cii.add_trace(go.Scatter(x=years, y=[5.0, 4.8, 4.6, 4.4, 4.2], name='Unmodified', line=dict(color='crimson')))
        fig_cii.add_trace(go.Scatter(x=years, y=[4.2, 4.0, 3.8, 3.6, 3.4], name='Optimized', line=dict(color='limegreen')))
        fig_cii.update_layout(template="plotly_dark", height=250)
