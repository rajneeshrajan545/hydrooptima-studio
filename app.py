...
if check_password():
    ...
    # Clean standard initial placeholders state fallback data setups
    ...
    with col1:
        ...
        if input_mode == "Automated CSV Upload":
            ...
            if uploaded_file is not None:
                try:
                    ...
                    st.success("🎯 Towing tank spreadsheet parameters loaded successfully into active state!")
                except Exception as e:
                    st.error(f"Failed to read CSV payload headers: {e}")
            else:
        ...
        if auto_optimize:
            st.info("⚡ **Autopilot Mode Active:** Slider parameter inputs have been overridden by automated optimization algorithms to maximize cash recovery.")
